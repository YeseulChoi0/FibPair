...
public class FibPair {
    ...
    private static Pair<Integer> nextPairAfter(Pair<Integer> pair) {
        Pair<Integer> nextPair = new Pair<Integer>(pair.getFirst() + pair.getSecond(), pair.getFirst());
        return nextPair;
    ...
    private static Integer fib( int n) {
      int counter = 1;
      Pair<Integer> currentPair= new Pair<Integer>(1, 0);
      while (counter < n){
        currentPair = nextPairAfter(currentPair);
        counter++;
      }
